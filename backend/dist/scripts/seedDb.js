...
const connectDB = async () => {
    try {
        await mongoose_1.default.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/ticketing-system');
...
const clearData = async () => {
    try {
        ...
        await venue_model_1.default.deleteMany({});
...
const createAdminUser = async () => {
    try {
        ...
        const existingAdmin = await user_model_1.default.findOne({ email: 'admin@example.com' });
        if (existingAdmin) {
...
const createVenues = async () => {
    try {
        ...
        const createdVenues = await venue_model_1.default.insertMany(venues);
...
// Create mock events
const createEvents = async (admin, venues) => {
    try {
        ...
        const createdEvents = await event_model_1.default.insertMany(events);
